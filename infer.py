...
def infer(args):
    ...
    controlnet = ControlNetModel.from_pretrained(
        os.path.join(args.checkpoint, "controlnet"), torch_dtype=weight_dtype
    ).to(device)
    ...
    if os.path.exists(pretrained_unet_path) and "stable-diffusion" in args.base_model:
        pipe.unet = UNet2DConditionModel.from_pretrained(
            pretrained_unet_path, torch_dtype=weight_dtype
        ).to(device)
    ...
    net_d3dfr = (
        model_resnet_d3dfr.getd3dfr_res50(
            os.path.join(args.checkpoint, "third_party/d3dfr_res50_nofc.pth")
        )
        .eval()
        .to(device)
    )
    bfm_facemodel = bfm.BFM(
        focal=1015 * 256 / 224,
        image_size=256,
        bfm_model_path=os.path.join(args.checkpoint, "third_party/BFM_model_front.mat"),
    ).to(device)

    net_arcface = model_insightface_backbone.getarcface(
        os.path.join(args.checkpoint, "third_party/insightface_glint360k.pth")
    ).to(device)
    clip_image_processor = CLIPImageProcessor()

    # "laion/CLIP-ViT-H-14-laion2B-s32B-b79K"
    net_vision_encoder = CLIPVisionModel.from_pretrained(
        os.path.join(args.checkpoint, "vision_encoder")
    ).to(device)
    # net_vision_encoder.vision_model.post_layernorm.requires_grad_(False)
    net_image2token = Image2Token(
        visual_hidden_size=net_vision_encoder.vision_model.config.hidden_size,
        text_hidden_size=768,
        max_length=77,
        num_layers=3,
    ).to(device)
    net_image2token.load_state_dict(
        torch.load(os.path.join(args.checkpoint, "net_image2token.pth"))
    )

    net_id2token = ID2Token(
        id_dim=512, text_hidden_size=768, max_length=77, num_layers=3
    ).to(device)
    net_id2token.load_state_dict(
        torch.load(os.path.join(args.checkpoint, "net_id2token.pth"))
    )

    net_seg_res18 = model_seg_unet.UNet().eval().to(device)
    net_seg_res18.load_state_dict(
        torch.load(os.path.join(args.checkpoint, "net_seg_res18.pth"))
    )
    app = FaceAnalysis(
        name="antelopev2",
        root=os.path.join(args.checkpoint, "third_party"),
        providers=["CUDAExecutionProvider", "CPUExecutionProvider"],
    )

    app.prepare(ctx_id=0, det_size=(512, 512))  # for CelebA-HQ

    net_seg = (
        model_parsing.get_face_parsing(
            os.path.join(args.checkpoint, "third_party/79999_iter.pth")
        )
        .eval()
        .to(device)
    )
    ...
    drive_img_list = [
        x
        for x in os.listdir(args.target)
        if x.endswith(".png") or x.endswith(".jpg") or x.endswith(".jpeg")
    ]
    drive_img_list.sort()

    max_imgs = min(len(src_img_list), len(drive_img_list))
    print(f"evaluating on {max_imgs} images...")

    for img_num in range(max_imgs):
        # for src_im_file in src_img_list:
        src_im_file = src_img_list[img_num]

        src_im_name = src_im_file.split(".")[0]
        src_img_path = os.path.join(args.source, src_im_file)
        src_im_pil = Image.open(src_img_path).convert("RGB")  # .resize((512, 512))

        # part where face is cropped
        ...
        if args.crop_ratio > 0:
            ...
            bbox_pts4 = np.array(
                [[x1, y1], [x1, y2], [x2, y2], [x2, y1]], dtype=np.float32
            )
        else:
            # original box
        ...
        image_src_crop256 = (
            pil2tensor(src_im_crop256_pil).view(1, 3, 256, 256).to(device)
        )
        images_src = (
            pil2tensor(src_im_pil)
            .view(1, 3, test_image_size, test_image_size)
            .to(device)
        )
        clip_input_src_tensors = (
            clip_image_processor(images=src_im_pil, return_tensors="pt")
            .pixel_values.view(-1, 3, 224, 224)
            .to(device)
        )

        for drive_im_file in [
            drive_img_list[img_num]
        ]:  # placeholder! evaluating on only 1 img
            drive_im_name = drive_im_file.split(".")[0]
            print(src_im_name, drive_im_name)
            drive_img_path = os.path.join(args.target, drive_im_file)
            drive_im_pil = Image.open(drive_img_path).convert(
                "RGB"
            )  # .resize((512, 512))
            ...
            face_info = sorted(
                face_info,
                key=lambda x: (x["bbox"][2] - x["bbox"][0]) * x["bbox"][3]
                - x["bbox"][1],
            )[
                -1
            ]  # only use the maximum face
            ...
            clip_input_tar_tensors = (
                clip_image_processor(images=drive_im_pil, return_tensors="pt")
                .pixel_values.view(-1, 3, 224, 224)
                .to(device)
            )
            ...
            face_masks_tar = (
                net_seg_res18(torch.cat([images_src, im_pts70], dim=1)) > 0.5
            ).float()
            controlnet_image = im_pts70 * face_masks_tar + images_src * (
                1 - face_masks_tar
            )  # tar for reconstruction
            ...
            faceid = net_arcface(
                F.interpolate(image_src_crop256, [128, 128], mode="bilinear")
            )
            encoder_hidden_states_src = net_id2token(faceid).to(dtype=weight_dtype)

            last_hidden_state = net_vision_encoder(
                clip_input_src_tensors
            ).last_hidden_state
            controlnet_encoder_hidden_states_src = net_image2token(
                last_hidden_state
            ...
            controlnet_encoder_hidden_states_tar = net_image2token(
                last_hidden_state
            ).to(dtype=weight_dtype)

            empty_prompt_token = (
                torch.load("empty_prompt_embedding.pth")
                .view(1, 77, 768)
                .to(dtype=weight_dtype)
                .to(device)
            )
            ...
            ##########    face swapping   ##########
            accurate_mask_path = os.path.join(
                args.target, "mask", drive_im_name + ".png"
            )
            if os.path.isfile(accurate_mask_path):
                print("use precomputed mask")
                mask_image_loaded = Image.open(accurate_mask_path)
                mask_numpy_loaded = np.array(mask_image_loaded).astype(np.float32) * (
                    20.0 / 255
                )
                masks_tar = (
                    torch.round(
                        torch.from_numpy(mask_numpy_loaded).unsqueeze(0).unsqueeze(0)
                    )
                    .long()
                    .to(res_tensor)
                )
                ...
                # net_seg = model_parsing.get_face_parsing(os.path.join(args.checkpoint, 'third_party/79999_iter.pth')).eval().to(device) # init before cycle
                seg_pred = net_seg(images_tar)[0]
                masks_tar = torch.argmax(
                    F.interpolate(
                        seg_pred, [test_image_size, test_image_size], mode="bilinear"
                    ),
                    dim=1,
                    keepdim=True,
                ...
                face_masks_tar = (
                    ((masks_tar > 0) & (masks_tar < 7))
                    | ((masks_tar > 9) & (masks_tar < 14))
                ).float() - (masks_tar == 6).float()
                face_masks_tar_withear = (
                    ((masks_tar > 0) & (masks_tar < 14)).float()
                    - (masks_tar == 9).float()
                    - (masks_tar == 6).float()
                )
                occ_mask = (
            ...
            face_masks_tar = torch.max(
                face_masks_tar_withear,
                F.max_pool2d(face_masks_tar, kernel_size=65, stride=1, padding=32),
            )
            face_masks_tar = face_masks_tar * (1 - occ_mask)
            face_masks_tar = F.max_pool2d(
                face_masks_tar, kernel_size=5, stride=1, padding=2
            )
            ...
            # im_rgb_pil.save(os.path.join(save_concat_path, f'{src_im_name}_{drive_im_name}_{img_num}.png'), quality=100)
            # im_rgb_pil.save(os.path.join(save_concat_path, f'{img_num}.png'), quality=100)

            images_res_pil = Image.fromarray(
                (swap_res_tensor[0] * 127.5 + 128)
                .cpu()
                .numpy()
                .astype("uint8")
                .transpose(1, 2, 0)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-o", "--output", type=str, default="./Celeba-HQ_val")
    parser.add_argument(
        "-s",
        "--source",
        type=str,
        default="/home/maignatov_2/REFace/dataset/FaceData/CelebAMask-HQ/Val",
    )
    parser.add_argument(
        "-t",
        "--target",
        type=str,
        default="/home/maignatov_2/REFace/dataset/FaceData/CelebAMask-HQ/Val_target",
    )
    parser.add_argument("-r", "--crop_ratio", type=float, default=0.81)
    parser.add_argument("-d", "--cache_dir", type=str, default="./hub")
    parser.add_argument(
        "-b", "--base_model", type=str, default="runwayml/stable-diffusion-v1-5"
    )
